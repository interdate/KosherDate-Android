...
var app = { 
				...
				403: function(response, textStatus, xhr){
					...
					if(app.exit===false){
						navigator.notification.alert(							
							'מידע הכנסה הוא שגוי, אנא נסה שוב',
    ...
    persistGcmDeviceId: function(){
    	$.ajax({				
			url: 'http://m.kosherdate.co.il/api/v1/user/gcmDeviceId',
			type: 'Post',
			data: JSON.stringify({			
	...
	getUsers: function(){
		...
		if(app.action == 'getOnlineNow'){					
			...
			app.requestUrl = 'http://m.kosherdate.co.il/api/v1/users/search/religionId:'+religionId+'/countryRegionId:'+countryRegionId+'/age:'+ageFrom+'-'+ageTo+'/userGender:'+userGender+'/nickName:'+nickName+'/count:'+app.itemsPerPage+'/page:'+app.pageNumber+'/sort:'+app.sort;
...
/*	getSexPreference: function(){
	...
	getRegions: function(){
		$.ajax({
			...
			success: function(list, status, xhr){	
				...
				var html = '<select name="regionCode" data-iconpos="left">';
	...
	getReligions: function(){
		$.ajax({
			...
			success: function(list, status, xhr){	
				//alert(JSON.stringify(list));
				var html = '<select name="religionId" data-iconpos="left">';
				if(app.currentPageId == 'search_form_page'){
					html = html + '<option value="">לא חשוב</option>';
				}				
	...
	formIsValid: function(){
		...
		if($('#userCityName').val().length == 0){
			$('#userCityName').css({'background':'red'});
			if(text==''){
				text='אנא הזינו עיר';
				$('#userCityName').focus();
			}
		...
		if($('#userfName').val().length == 0){
			$('#userfName').css({'background':'red'});
			if(text==''){
		...
		if($('#userlName').val().length == 0){
			$('#userlName').css({'background':'red'});
			if(text==''){
